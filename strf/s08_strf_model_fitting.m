function s08_strf_model_fitting(paths,strf_ps,lambda_info)
%% adding path ...
addpath(paths.mtrf_toolbox);
input_root_path = paths.strf_sets_seperated_data;
output_root_path = paths.strf_model_fitting_results;

%% training parameters ...
tmin = strf_ps.training_tmin;
tmax = strf_ps.training_tmax;
direction = 1;
fs= strf_ps.fs;

%% training ...
all_strf_data = dir(fullfile(input_root_path,'*strf_cv_dataset*.mat'));
for subj_i=1:length(all_strf_data)
    load(fullfile(input_root_path,all_strf_data(subj_i).name)); % strf_cv ...
    ...
    tmp_subj = all_strf_data(subj_i).name(1:8);
    ...
    for trial_i = 1:length(tmp_training_data)
        ...
        for band_i = 1:length(unique_bands)
            ...
            tmp_model = mTRFtrain(tmp_stim,tmp_resp,...
                fs,direction,tmin,tmax,tmp_lambda,'method','ridge',...
                'zeropad',0);
